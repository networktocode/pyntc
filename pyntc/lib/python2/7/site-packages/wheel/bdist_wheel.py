...
def get_flag(var, fallback, expected=True, warn=True):
    ...
    if val is None:
        if warn:
            warnings.warn(
                "Config variable '{0}' is unset, Python ABI tag may " "be incorrect".format(var), RuntimeWarning, 2
            )
        return fallback
    return val == expected
...
def get_abi_tag():
    ...
    if not soabi and impl in ("cp", "pp") and hasattr(sys, "maxunicode"):
        ...
        if get_flag(
            ...
            m = "m"
        if get_flag(
            "Py_UNICODE_SIZE", sys.maxunicode == 0x10FFFF, expected=4, warn=(impl == "cp" and sys.version_info < (3, 3))
...
class bdist_wheel(Command):
    ...
    def license_paths(self):
        ...
        if "license_file" in metadata:
            warnings.warn(
                'The "license_file" option is deprecated. Use ' '"license_files" instead.', DeprecationWarning
            )
