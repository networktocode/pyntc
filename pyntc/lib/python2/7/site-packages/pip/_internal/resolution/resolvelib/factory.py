...
class Factory(object):
    ...
    def _iter_found_candidates(
        ...
        found = self._finder.find_best_candidate(project_name=name, specifier=specifier, hashes=hashes,)
        for ican in found.iter_applicable():
            if ican.version == installed_version and installed_candidate:
                candidate = installed_candidate
            else:
                candidate = self._make_candidate_from_link(
                    link=ican.link, extras=extras, template=template, name=name, version=ican.version,
                )
    ...
    def make_requirement_from_install_req(self, ireq, requested_extras):
        ...
        if not ireq.match_markers(requested_extras):
            logger.info(
                "Ignoring %s: markers '%s' don't match your environment", ireq.name, ireq.markers,
            )
            return None
        if not ireq.link:
        ...
        if ireq.link.is_wheel:
            ...
            if not wheel.supported(self._finder.target_python.get_tags()):
                msg = "{} is not a supported wheel on this platform.".format(wheel.filename,)
    ...
    def get_dist_to_uninstall(self, candidate):
        ...
        if running_under_virtualenv() and dist_in_site_packages(dist):
            raise InstallationError(
                ...
                "lack sys.path precedence to {} in {}".format(dist.project_name, dist.location,)
            )
        return None
    ...
    def _report_requires_python_error(
        ...
        message = message_format.format(
            package=template.name, version=self._python_candidate.version, specifier=str(requirement.specifier),
        )
