...
def _check_link_requires_python(
    ...
    try:
        ...
        logger.debug(
            "Ignoring invalid Requires-Python (%r) for link: %s", link.requires_python, link,
        )
    else:
        if not is_compatible:
            ...
            if not ignore_requires_python:
                logger.debug(
                    "Link requires a different Python (%s not in: %r): %s", version, link.requires_python, link,
                )
                return False
...
class LinkEvaluator(object):
    ...
    def evaluate_link(self, link):
        ...
        supports_python = _check_link_requires_python(
            link, version_info=self._target_python.py_version_info, ignore_requires_python=self._ignore_requires_python,
        )
...
class CandidateEvaluator(object):
    ...
    def get_applicable_candidates(
        ...
        filtered_applicable_candidates = filter_unallowed_hashes(
            candidates=applicable_candidates, hashes=self._hashes, project_name=self._project_name,
        )
    ...
    def compute_best_candidate(
        ...
        return BestCandidateResult(
            candidates, applicable_candidates=applicable_candidates, best_candidate=best_candidate,
        )
...
class PackageFinder(object):
    ...
    def create(
        ...
        candidate_prefs = CandidatePreferences(
            prefer_binary=selection_prefs.prefer_binary, allow_all_prereleases=selection_prefs.allow_all_prereleases,
        )
