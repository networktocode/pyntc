...
def _get_html_page(link, session=None):
    ...
    try:
        ...
        logger.warning(
            "Skipping page %s because it looks like an archive, and cannot " "be checked by a HTTP HEAD request.", link,
        )
...
class LinkCollector(object):
    ...
    def create(cls, session, options, suppress_no_index=False):
        ...
        if options.no_index and not suppress_no_index:
            logger.debug(
                "Ignoring indexes: %s", ",".join(redact_auth_from_url(url) for url in index_urls),
            )
            index_urls = []
