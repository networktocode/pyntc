...
def _should_build(
    ...
    if not check_binary_allowed(req):
        logger.info(
            "Skipping wheel build for %s, due to binaries " "being disabled for it.", req.name,
        )
        return False
    ...
    if not req.use_pep517 and not is_wheel_installed():
        ...
        logger.info(
            "Using legacy 'setup.py install' for %s, " "since package 'wheel' is not installed.", req.name,
        )
        return False
...
def build(
    ...
    # Build the wheels.
    logger.info(
        "Building wheels for collected packages: %s", ", ".join(req.name for req in requirements),  # type: ignore
    )
    ...
    if build_successes:
        logger.info(
            "Successfully built %s", " ".join([req.name for req in build_successes]),  # type: ignore
        )
    if build_failures:
        logger.info(
            "Failed to build %s", " ".join([req.name for req in build_failures]),  # type: ignore
        )
