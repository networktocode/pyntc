...
def direct_url_from_link(link, source_dir=None, link_is_in_wheel_cache=False):
    ...
    if link.is_vcs:
        ...
        return DirectUrl(
            url=url,
            info=VcsInfo(vcs=vcs_backend.name, commit_id=commit_id, requested_revision=requested_revision,),
        ...
        return DirectUrl(
            url=link.url_without_fragment, info=ArchiveInfo(hash=hash), subdirectory=link.subdirectory_fragment,
        )
...
def dist_get_direct_url(dist):
    ...
    try:
        ...
        logger.warning(
            "Error parsing %s for %s: %s", DIRECT_URL_METADATA_NAME, dist.project_name, e,
        )
        return None
