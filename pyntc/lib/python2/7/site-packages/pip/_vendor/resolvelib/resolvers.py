...
class InconsistentCandidate(ResolverException):
    ...
    def __str__(self):
        return "Provided candidate {!r} does not satisfy {}".format(
            self.candidate, ", ".join(repr(r) for r in self.criterion.iter_requirement()),
        )
...
class Criterion(object):
    ...
    * `candidates` is a collection containing all possible candidates deducted
    ...
    def from_requirement(cls, provider, requirement, parent):
        ...
        criterion = cls(
            candidates=candidates, information=[RequirementInformation(requirement, parent)], incompatibilities=[],
        )
        if not candidates:
...
class Resolution(object):
    ...
    def _attempt_to_pin_criterion(self, name, criterion):
        ...
        for candidate in criterion.candidates:
            ...
            satisfied = all(self._p.is_satisfied_by(r, candidate) for r in criterion.iter_requirement())
            if not satisfied:
    ...
    def resolve(self, requirements, max_rounds):
        ...
        for round_index in range(max_rounds):
            ...
            unsatisfied_criterion_items = [
                item for item in self.state.criteria.items() if not self._is_current_pin_satisfying(*item)
            ]
            ...
            name, criterion = min(unsatisfied_criterion_items, key=self._get_criterion_item_preference,)
